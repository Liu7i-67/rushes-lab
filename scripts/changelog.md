...
- 部署 liuqi 分支到内网 dev(hh2,192.168.110.221:8090):代码 9b71dd7 + SPA 新 bundle + 容器装 pillow-heif 后重启 ms-api/ms-worker;验收 auth/me 401 ✓ / SPA 新 hash ✓ / trash 端点 401(存在)✓ / worker 注册 livp 任务 ✓。**部署事故披露**:deploy_lan.sh 的 tar 兜底分支(Git Bash)用锚定根路径的 `./data` 排除项,没拦住 `poc/minio/data`(本机 MinIO 实际数据目录),把本地 2.8MB MinIO 数据 rsync 进了内网 dev —— ctime 实证影响面仅 `.minio.sys` 内部件(usage 缓存 + 本地遗留的未完成 multipart + tmp,共 26 个文件,运行无影响),业务对象零改动、远端独有数据完好(rsync 无 --delete)。根因已修:排除项改裸名(任意层级匹配)+ 显式 `poc/minio/data*` + 第二段 rsync 再加一道 MinIO 数据排除(双保险);另注:dev 环境 nginx 把 `/healthz` 路由到 MinIO(既有配置,与应用无关,健康检查走 `/api/v1/auth/me`)(919e9c3)
- 第四轮审查(合并门)收口(1×P1 + 2×P2):① `update_asset_meta`(打标/备注)补 StaleDataError → 404 —— 打标与并发 hard purge 竞速的 0 行 UPDATE 同样抛异常(容器内实证,全仓该模式唯一漏网点),照抄 restore 的四行修法,至此本迭代所有「已加载实例改属性再 commit」的写路径竞速全部收敛为幂等 404;② 删夹 409 文案一律去计数 —— 删夹门槛(can_upload)低于回收站可见门槛(can_admin),仅 uploader 者看不到回收站却从报文拿精确计数;数字对能看回收站的 admin 是冗余(角标/弹窗都有),普通夹文案改"回收站内还有已删除文件…"并加无数字断言;③ 前端清空回收站空窗口分支的 refetch 补异常兜底(网络失败给提示不再静默)。审查同时确认 d0e0000 三项修复全部有效、容器内 41 例全过、黑盒全链(回收站/livp/敏感夹)零残留 —— **无 P0,可合并**(152a082)
